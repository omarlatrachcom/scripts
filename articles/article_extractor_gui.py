...
class IslamOnlinePageParser(HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        self.cover_image = ""
        self.cover_alt = ""
        self.in_books_category = False
        self.in_sharia_category = False
    ...
    def _capture_metadata(self, tag: str, attrs: dict[str, str | None]) -> None:
        ...
        content = attrs.get("content") or ""
        if tag == "meta":
            if prop == "article:published_time" and not self.published:
                self.published = content
            elif prop == "og:image" and not self.cover_image:
                self.cover_image = content
            elif prop == "og:image:alt" and not self.cover_alt:
                self.cover_alt = content
            elif prop == "og:title" and not self.title:
                self.title = _clean_inline(content)
        elif tag == "link":
    ...
    def handle_starttag(self, tag: str, attrs_list: list[tuple[str, str | None]]) -> None:
        ...
        if not self._breadcrumb_depth and tag == "nav" and attrs.get("id") == "breadcrumb":
            self._breadcrumb_depth = 1
        elif self._breadcrumb_depth and tag not in self.VOID_TAGS:
            self._breadcrumb_depth += 1
        if tag == "a" and self._breadcrumb_depth:
            href = attrs.get("href") or ""
            if "/category/books/" in href:
                self.in_books_category = True
            if "/category/sharia/" in href:
                self.in_sharia_category = True

        if not self._body_depth and tag == "article" and (
            attrs.get("id") == "article" or attrs.get("itemprop") == "articleBody"
        ):
            self._body_depth = 1
            return
        if self._body_depth:
        ...
        if tag == "h1" and "headline" in itemprop.split():
            self._title_depth = 1
    ...
    def handle_startendtag(
        ...
        if (
            tag == "a"
            and self._breadcrumb_depth
            and "/category/books/" in (attrs.get("href") or "")
        ):
            self.in_books_category = True
        if (
            tag == "a"
            and self._breadcrumb_depth
            and "/category/sharia/" in (attrs.get("href") or "")
        ):
            self.in_sharia_category = True
        if self._body_depth:
            raw = self.get_starttag_text()
    ...
    def handle_endtag(self, tag: str) -> None:
        if self._body_depth:
            if self._body_depth == 1 and tag == "article":
                self._body_depth = 0
            else:
        ...
        if self._title_depth:
            ...
            if not self._title_depth:
                captured = _clean_inline("".join(self._title_parts))
                if captured:
                    self.title = captured
...
class InfoQPodcastPageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs_list: list[tuple[str, str | None]]) -> None:
        ...
        if not self._podcast_depth and tag == "article" and attrs.get("data-type") == "podcast":
            self.is_podcast = True
            self._podcast_depth = 1
            return
        if self._podcast_depth and tag not in self.VOID_TAGS:
            self._podcast_depth += 1

        if self._capture_depth:
            if self._capture_skip_depth:
                if tag not in self.VOID_TAGS:
                    self._capture_skip_depth += 1
                return
            if tag == "div" and "author-section-full" in _class_tokens(attrs):
                self._capture_skip_depth = 1
                return
            raw = self.get_starttag_text()
            if raw:
        ...
        if (
            ...
            return
        if self._podcast_depth and tag == "iframe" and attrs.get("id") == "player":
            self.player_url = attrs.get("src") or ""
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self._capture_depth:
            if self._capture_skip_depth:
                self._capture_skip_depth -= 1
                if self._podcast_depth:
                    self._podcast_depth -= 1
                return
            if self._capture_depth == 1 and tag == "div":
...
class IslamOnlineBooksAdapter:
    ...
    def extract(self, url: str, progress: Callable[[str], None]) -> ExtractedArticle:
        progress("Downloading the IslamOnline article page…")
        page = fetch_text(url)
        ...
        if not parser.in_books_category:
            raise ExtractionError(
                "This IslamOnline extractor is configured for articles in the Books category."
            )
        ...
        title = parser.title or "IslamOnline article"
...
def _infoq_direct_audio_url(player_url: str, feed_xml: str) -> str:
    ...
    for item in root.iter("item"):
        ...
        if track_id not in guid:
            continue
        enclosure = item.find("enclosure")
        source = enclosure.get("url", "") if enclosure is not None else ""
        if source.startswith("http://dts.podtrac.com/"):
            source = "https://" + source.removeprefix("http://")
        return source
    return ""


class InfoQPodcastAdapter:
    """Dedicated article-only adapter for InfoQ podcast pages and transcripts."""
    ...
    def extract(self, url: str, progress: Callable[[str], None]) -> ExtractedArticle:
        progress("Downloading the InfoQ podcast page…")
        page = fetch_text(url)
        parser = InfoQPodcastPageParser()
        parser.feed(page)
        parser.close()
        schema = parser.article_schema()
        title = _clean_inline(str(schema.get("headline") or ""))
        if not parser.is_podcast or not title or not parser.content_fragments:
            raise ExtractionError(
                "InfoQ returned no recognizable public podcast article. Make sure the URL "
                "points to an individual /podcasts/ page with public editorial content."
            )

        author_names = self._schema_people(schema.get("author"))
        contributor_names = self._schema_people(schema.get("contributor"))
        direct_audio = ""
        if parser.player_url:
            progress("Resolving the direct podcast audio from InfoQ's public feed…")
            try:
                direct_audio = _infoq_direct_audio_url(
                    parser.player_url, fetch_text(INFOQ_PODCAST_FEED)
                )
            except ExtractionError as exc:
                progress(f"Warning: direct podcast audio could not be resolved: {exc}")

        blocks: list[str] = []
        media: list[MediaReference] = []
        player_added = False
        for fragment in parser.content_fragments:
            fragment_blocks, fragment_media = html_to_blocks(fragment, len(media) + 1)
            is_transcript = any(
                block.lstrip("# ").strip().lower() == "transcript"
                for block in fragment_blocks
                if block.startswith("#")
            )
            if is_transcript and parser.player_url and not player_added:
                marker = f"IMAGE-{len(media) + 1:02d}"
                source = direct_audio or parser.player_url
                kind = "audio" if direct_audio else "iframe"
                blocks.append(marker)
                media.append(MediaReference(marker, kind, source, title, "podcast player"))
                player_added = True
                # The fragment was numbered before the player was inserted.
                fragment_blocks, fragment_media = html_to_blocks(fragment, len(media) + 1)
            blocks.extend(fragment_blocks)
            media.extend(fragment_media)
        if parser.player_url and not player_added:
            marker = f"IMAGE-{len(media) + 1:02d}"
            source = direct_audio or parser.player_url
            kind = "audio" if direct_audio else "iframe"
            blocks.append(marker)
            media.append(MediaReference(marker, kind, source, title, "podcast player"))
        if not blocks:
            raise ExtractionError("The InfoQ podcast contained no readable public content.")
        ...
        subtitle = (
            "Podcast with " + ", ".join(contributor_names) if contributor_names else ""
        )
        return ExtractedArticle(
            title=title,
            subtitle=subtitle,
...
WEBSITE_EXTRACTORS: tuple[WebsiteExtractorDefinition, ...] = (
    ...
    WebsiteExtractorDefinition(
        ...
        homepage="https://islamonline.net/category/books/",
        description=(
            "Extract public IslamOnline book reviews and summaries as structured article text. "
            "Comments are not extracted."
        ),
        example_url=(
            "https://islamonline.net/%d8%b9%d8%b1%d8%b6-%d9%83%d8%aa%d8%a7%d8%a8-"
            "%d8%aa%d9%88%d8%b8%d9%8a%d9%81-%d8%a7%d9%84%d8%b0%d9%83%d8%a7%d8%a1-"
            "%d8%a7%d9%84%d8%aa%d9%88%d9%84%d9%8a%d8%af%d9%8a-%d9%81%d9%8a-%d8%a7%d9%84%d8%a7%d8%ac/"
        ),
    ...
    WebsiteExtractorDefinition(
        key="infoq-podcasts",
        display_name="InfoQ",
        homepage="https://www.infoq.com/podcasts/",
        description=(
            "Extract complete InfoQ podcast articles, key takeaways, audio, and transcripts. "
            "Comments are not extracted."
        ),
