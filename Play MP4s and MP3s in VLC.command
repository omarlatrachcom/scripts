...
done < <(find -s "$BASE_DIR" -type f \( -iname "*.mp4" -o -iname "*.mp3" -o -iname "*.mkv" \) -print0)

if [ ${#FILES[@]} -eq 0 ]; then
  echo "No MP4, MP3, or MKV files found in:"
  echo "$BASE_DIR"
  read -k 1 "?Press any key to close..."
  exit 0
fi

echo "Playing ${#FILES[@]} MP4/MP3/MKV file(s) in order..."
echo
