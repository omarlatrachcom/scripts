...
REQUESTED_DIR="${3:-}"
EXTRA_ITEM_STRICT_LIMIT="${4:-0}"

if [[ ! "$MAX_SECONDS" =~ '^[0-9]+$' || "$MAX_SECONDS" -le 0 || -z "$MAX_DURATION_LABEL" ||
      ! "$EXTRA_ITEM_STRICT_LIMIT" =~ '^[0-9]+$' ]]; then
  echo "Usage: ${0:t} MAX_SECONDS DURATION_LABEL [FOLDER] [EXTRA_ITEM_STRICT_LIMIT]"
  exit 2
fi

if (( EXTRA_ITEM_STRICT_LIMIT > 0 && EXTRA_ITEM_STRICT_LIMIT <= MAX_SECONDS )); then
  echo "EXTRA_ITEM_STRICT_LIMIT must be greater than MAX_SECONDS."
  exit 2
fi
